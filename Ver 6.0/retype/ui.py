"""
The ReType 6.0 window - native Windows look (default ttk theme).

Pick a typist persona, choose a source (the text box or your clipboard), and:
  ...
  * Global hotkey - press it in ANY window to start/stop a run without coming
              back to ReType (handy with the Clipboard source: copy -> focus
              the field -> hotkey -> it types your clipboard)

A live "cardiogram" strip shows instantaneous speed. Pause / Stop work mid-type;
the mouse-to-a-corner failsafe aborts a real run instantly.
"""
...
class App:
    def __init__(self, root):
        ...
        # Global hotkey: start/stop a run from any window.
        self.hotkeys = hotkey.HotkeyWatcher(root, self._current_combo,
                                            self._hotkey_toggle)
        self.hotkeys.start()

    # ------------------------------------------------------------------ UI ---
    def _build_ui(self):
        self.root.title("ReType 6.0")
        ...
        qrow = ttk.LabelFrame(outer, text="Source & global hotkey", padding=6)
        qrow.pack(fill="x", pady=(0, 6))
        self.source = tk.StringVar(value="textbox")
        ttk.Radiobutton(qrow, text="Type the text box", variable=self.source,
                        value="textbox").grid(row=0, column=0, sticky="w")
        ttk.Radiobutton(qrow, text="Type the clipboard", variable=self.source,
                        value="clipboard").grid(row=0, column=1, sticky="w", padx=(12, 0))
        ttk.Label(qrow, text="Hotkey").grid(row=1, column=0, sticky="w", pady=(4, 0))
        self.hotkey_name = tk.StringVar(value=hotkey.DEFAULT_HOTKEY)
        hk_box = ttk.Combobox(qrow, textvariable=self.hotkey_name, state="readonly",
                              values=list(hotkey.HOTKEYS), width=16)
        hk_box.grid(row=1, column=1, sticky="w", padx=(12, 0), pady=(4, 0))
        hk_box.bind("<<ComboboxSelected>>", lambda e: self._update_hk_hint())
        self.hk_hint = ttk.Label(qrow, text="", foreground="#555",
                                 font=("Segoe UI", 8, "italic"))
        self.hk_hint.grid(row=2, column=0, columnspan=2, sticky="w", pady=(4, 0))
        self._update_hk_hint()
    ...
    def _current_combo(self):
        return hotkey.HOTKEYS.get(self.hotkey_name.get())

    def _update_hk_hint(self):
        self.hk_hint.config(
            text=f"Press {self.hotkey_name.get()} in any window to start / stop a run.")

    def _get_content(self, *, quiet=False):
        """Text to type, from the chosen source (text box or clipboard)."""
        if self.source.get() == "clipboard":
            try:
        ...
        if not content and not quiet:
            messagebox.showinfo("ReType", "There's no text to type "
                                "(the clipboard is empty?)." if self.source.get() ==
                                "clipboard" else "There's no text to type.")
        return content
    ...
    def start_real(self, countdown=None):
        if self._busy():
            return
        ...
        secs = self.countdown.get() if countdown is None else countdown
        self._countdown(max(0, secs))

    def _hotkey_toggle(self):
        """Global hotkey: stop a run in progress, or start one into the focused window."""
        if self._busy():
            self.stop()
            return
        if not self._get_content(quiet=True):
            return          # nothing to type - never pop a modal over another app
        # The user is already in their target window; use a short lead so the
        # hotkey's modifier keys are released before we start typing.
        self.start_real(countdown=1)
